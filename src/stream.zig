...
pub const OpensslStream = struct {
    ...
    pub const Reader = struct {
        ...
        fn readVec(io_r: *Io.Reader, data: [][]u8) Io.Reader.Error!usize {
            ...
            const result = openssl.SSL_read_ex(r.stream.ssl, dest[0].ptr, @intCast(dest[0].len), &n);
            if (result <= 0) {
                r.stream.valid = false;
    ...
    pub fn connect(io: Io, opts: Opts) Error!PlainStream {
        const stream = try blk: {
            ...
            if (host.len > 0 and host[0] == '/') {
                if (comptime Io.net.has_unix_sockets == false or posix.AF == void) {
